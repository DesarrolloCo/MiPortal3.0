<div class="card">
    <div class="card-body">
        <h4 class="card-title"><i class="mdi mdi-cake text-warning"></i> Cumpleaños Próximos</h4>
        <h6 class="card-subtitle">Celebremos juntos - {{ \Carbon\Carbon::now()->locale('es')->isoFormat('MMMM') }} y {{ \Carbon\Carbon::now()->addMonth()->locale('es')->isoFormat('MMMM') }} del {{ \Carbon\Carbon::now()->year }}</h6>

        @if($cumpleanos->count() > 0)
            <div class="m-t-20" style="max-height: 400px; overflow-y: auto;">
                @foreach($cumpleanos->take(10) as $item)
                <div class="d-flex align-items-center border-bottom pb-3 pt-3 {{ $item['es_hoy'] ? 'bg-light-success' : '' }}">
                    ...
                    <div class="mr-3">
                        ...
                        @else
                        <div class="rounded-circle bg-{{ $item['es_hoy'] ? 'success' : ($item['es_esta_semana'] ? 'warning' : 'info') }} text-white d-inline-flex align-items-center justify-content-center"
                    ...
                    <div class="flex-grow-1">
                        ...
                        <small class="text-muted">
                            <i class="mdi mdi-calendar"></i> {{ $item['fecha'] }}
                            <span class="text-info">(cumple {{ $item['edad'] + 1 }} años)</span>
                        </small>
                    </div>
                    ...
                    <div class="ml-2">
                        @if($item['es_hoy'])
                            <span class="badge badge-success badge-pill">
                                <i class="mdi mdi-cake-variant"></i> ¡Hoy!
                            </span>
                ...
                @if($cumpleanos->count() > 10)
                    <div class="text-center pt-3">
                        <small class="text-muted">
                            <i class="mdi mdi-information"></i>
                            + {{ $cumpleanos->count() - 10 }} cumpleaños más próximos
                        </small>
                    </div>
                @endif
            </div>
            ...
            <div class="text-center mt-3 pt-3 border-top">
                <small class="text-muted">
                    <strong>{{ $cumpleanos->count() }}</strong> cumpleaños en total próximos
                </small>
            </div>
        @else
            <div class="text-center m-t-30 m-b-30">
                <i class="mdi mdi-emoticon-sad mdi-48px text-muted"></i>
                <p class="text-muted mt-2">No hay cumpleaños este mes</p>
            </div>
        @endif
    </div>
</div>
